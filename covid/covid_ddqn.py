...
parser.add_argument("--save_dir",
                    type=str,
                    default='C:\\Users\\kashann\\PycharmProjects\\adaptivFS\\covid\\covid_ddqn_models',
                    help="Directory for saved models")
parser.add_argument("--masked_images_dir",
                    type=str,
                    default='C:\\Users\\kashann\\PycharmProjects\\adaptivFS\\covid\\covid_masked_images',
...
class Agent(object):
    ...
    def get_action(self, states: np.ndarray, env,
        ...
        if np.random.rand() < eps and mode == 'training':
            ...
            # if all mask is 1 choose the seconde max element
            if argmax.item() == env.n_questions:
                flag = 1
                for i in range(len(mask)):
                    if mask[i] == 0:
                        flag = 0
                if flag == 1:
                    #take the second max element
                    # Find the indices of the top 2 elements along the last dimension
                    top_indices = torch.argsort(scores.data , dim=-1, descending=True)

                    # Extract the second highest index (top_indices[:, 1] contains the second highest indices)
                    argmax = top_indices[:, 1]
...
def show_sample_paths(n_patients, env, agent):
    ...
    for i in range(n_patients):
        ...
        # neg_patient = random.choice(n_patients)
        ...
        #
        #     if action != env.n_questions:
        #         print('Step: {}, Question: '.format(t + 1), env.question_names[action])
        #
        #     # take the action
        #     state, reward, done, guess = env.step(action, mode='test')
        #
        #     if guess != -1:
        #         print('Step: {}, Ready to make a guess: Prob(y=1)={:1.3f}, Guess: y={}, Ground truth: y=0'.format(t + 1,
        #                                                                                                           env.outcome_prob,
        #                                                                                                           guess))
        #
        #     if done:
